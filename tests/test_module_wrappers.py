...
class ModuleWrapperTests(unittest.TestCase):
    ...
    def test_llm_client_merges_transcript_context_through_adapter(self) -> None:
        ...
        self.assertIn("Respect existing content", kwargs["messages"][0]["content"])
        self.assertIn("Detect topic and context switches", kwargs["messages"][0]["content"])
        ...
        self.assertIn("full current editable transcript tail is shown", kwargs["messages"][1]["content"])
        self.assertIn("rewrite the new transcript sections into clean, grammatically correct text", kwargs["messages"][1]["content"])
